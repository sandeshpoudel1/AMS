...
class BdSourceController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($validator->fails()) {
            return response()->json([
                'success' => false,
                'error_code' => 'BD_SOURCE_VALIDATION_FAILED',
        ...
        return response()->json([
            'success' => true,
            'message' => 'BD source created successfully',
            'data' => $row,
        ], 201);
    }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $row = BdSource::find($id);

        if (!$row) {
            return response()->json([
                'success' => false,
                'error_code' => 'BD_SOURCE_NOT_FOUND',
                'message' => 'BD source not found.',
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'BD source updated successfully',
            'data' => $row,
        ]);
    }
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'BD source deleted successfully',
        ]);
    }
    ...
    private function ensureViewAccess(Request $request): ?JsonResponse
    {
        ...
        if (!in_array($role, ['admin', 'finance_officer', 'candidate_officer', 'documentation', 'documentation_head', 'account', 'superadmin', 'super_admin'], true)) {
            return response()->json([
                ...
                'message' => 'You do not have access to BD sources.',
            ], 403);
        }
    ...
    private function ensureAdminAccess(Request $request): ?JsonResponse
    {
        ...
        if (!in_array($role, ['admin', 'documentation', 'documentation_head', 'superadmin', 'super_admin'], true)) {
            return response()->json([
                'success' => false,
                'error_code' => 'AUTH_FORBIDDEN',
                'message' => 'Only admin or superadmin can manage BD sources.',
            ], 403);
        }
